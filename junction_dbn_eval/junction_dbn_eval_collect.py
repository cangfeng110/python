...
def main():
    ...
    column_labels = ['case_path', 'mean_brier_score', 'junction_id', 'exit_idx']
    for root, dirs, files in os.walk(case_path, topdown=False):
        for item in dirs:
            ...
            res_df.loc[-1] = [one_case_path, \
                              pd.read_pickle(eval_res_fname)['mean_brier_score'].iloc[0], \
                              pd.read_pickle(eval_res_fname)['junction_id'].iloc[0], \
                              pd.read_pickle(eval_res_fname)['exit_idx'].iloc[0]]
            data_df = data_df.append(res_df, ignore_index=True)

    output_path = os.path.join(output_path, "junction_dbn.eval_res")
    data_df.to_pickle(output_path)

    data_df.to_excel("/tmp/junction_dbn_eval_res.xlsx")
