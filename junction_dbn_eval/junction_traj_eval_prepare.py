...
def prepare(json_file, ucpf_fname, obj_id):
    ...
    processor.insert_key_val_to_json(["_MOD_uos_local_planner", \
                                      "prediction_replay"], 1)
    processor.insert_key_val_to_json(["_MOD_uos_local_planner", \
                                      "prediction_replay_ucpf_fname"], [ucpf_fname])
    processor.insert_key_val_to_json(["_MOD_uos_local_planner", \
                                      "dump_prediction_features"], 0)
    processor.insert_key_val_to_json(["_MOD_uos_local_planner", \
                                      "dump_motor_veh_junction_dbn_features"], 0)
    processor.insert_key_val_to_json(["_MOD_uos_local_planner", \
                                      "log_pred_traj"], 1)
    processor.insert_key_val_to_json(["_MOD_uos_local_planner", \
                                      "obj_id_for_traj_eval"], obj_id)
    processor.insert_key_val_to_json(["_MOD_uos_local_planner", \
                                      "enable_junction_predictor"], 1)
    processor.insert_key_val_to_json(["_MOD_uos_base", "log_mode"], 1)
...
def main():
    ...
    valid, ucpf_fname, obj_id = check_junction_case_validity(case_path)
    if not valid:
        print("Not a valid junction scenario case!")
        return

    prepare(config_file, ucpf_fname, obj_id)
